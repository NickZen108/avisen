...
p = Path('cloudflare/newsdesk/src/index.js')
s = p.read_text(encoding='utf-8')

anchor = '  { name: "Al Jazeera", url: "https://www.aljazeera.com/xml/rss/all.xml", source_class: "news", region: "WORLD", priority: 3, limit: 18 },\n'
addition = '''  { name: "Reuters World", url: "https://www.reuters.com/world/", mode: "html_links", link_path: "/world/", source_class: "core_news", region: "WORLD", priority: 4, limit: 20 },
...
'''
if 'Reuters World' not in s:
    if anchor not in s:
        raise SystemExit('feed anchor missing')
    s = s.replace(anchor, anchor + addition, 1)

helper_anchor = 'async function sha256Hex(text) {'
helpers = r'''
...
'''
if 'function extractXSyndicationItems(' not in s:
    if helper_anchor not in s:
        raise SystemExit('helper anchor missing')
    s = s.replace(helper_anchor, helpers + helper_anchor, 1)

old = '''    const xml = await response.text();
    const signals = extractItems(xml, feed);
'''
new = '''    const payload = await response.text();
...
elif 'feed.mode === "x_syndication"' not in s:
    raise SystemExit('fetchFeed anchor missing')

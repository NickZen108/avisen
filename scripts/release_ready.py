...
NON_EDITORIAL_AFTER_APPROVAL={
 'status','published_at','updated_at','scheduled_for','released_from_schedule_at','release_requested','publication','manual_review','manual_review_completed','workflow_state',
 'editorial_destination','related_news_slug','followup_type','weight','seo'
}
RETRYABLE={'language','image'}; MAX_RETRIES=3
def parse_iso(v):
...
def published_slugs():
 ...
 return out
def set_ticker(state,slug):
 state['ticker']={'slug':slug} if slug else {}; state.pop('ticker_text',None); return state
...
def route_failure(x,resume,reasons,stamp):
...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--now'); p.add_argument('--normalize-only',action='store_true'); a=p.parse_args(); now=parse_iso(a.now).astimezone(timezone.utc) if a.now else datetime.now(timezone.utc); stamp=now.replace(microsecond=0).isoformat().replace('+00:00','Z'); released=0; recovered=0; dropped=0; rows=[]
 for path in sorted(ARTICLE_DIR.glob('*.json')):
  if path.name.startswith('_'): continue
  x=load(path)
  if x.get('pipeline_version')!=2: continue
  reasons,resume=diagnose(x) if x.get('status') in {'ready','checking','editing','researching'} else ([],None)
  if x.get('status')=='ready' and x.get('release_requested') is True and reasons:
   outcome=route_failure(x,resume,reasons,stamp); path.write_text(json.dumps(x,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); repair_frontpage(x.get('slug')); recovered+=1; dropped+=outcome=='dropped'
...
def self_test():
 state={'ticker':{'slug':'old-slug'},'ticker_text':'Gammel ticker','lead':{'slug':'old-lead'},'rail':[],'narrow':[],'stack':[]}; set_ticker(state,'new-slug'); assert state['ticker']=={'slug':'new-slug'},state; assert 'ticker_text' not in state,state; set_ticker(state,None); assert state['ticker']=={}; print('release_ready self-test: PASS')

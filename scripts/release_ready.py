...
def main():
 ...
 for path in sorted(ARTICLE_DIR.glob('*.json')):
  ...
  if x.get('status')=='ready' and x.get('release_requested') is True and reasons:
   route_failure(x,resume,reasons,stamp); path.write_text(json.dumps(x,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); repair_frontpage(x.get('slug')); recovered+=1
 ...
 write_health(rows,stamp); print(f'Ready release: {released}; parked invalid tickets: {recovered}'); return 0
def self_test():

...
def reason_code(status,stage,reason):
 ...
 if s=='desk-recheck':return 'desk_staleness_or_scope'
...
def assessment(status,stage,reason):
 ...
 hard=['forelægg','right of reply','færre end to','kilde','fact check','fact-check','modsig','etik','duplicate','dublet']
 ...
 if any(x in r for x in hard) or s in {'ethics','fact-check'}:
  return ('blocked-correct','Blokering beskytter en reel journalistisk/etisk risiko.','Behold hård gate; undersøg kun om research kan løse manglen automatisk.')

...
def main():
 ...
 row={'at':payload.get('generated_at') or datetime.now(timezone.utc).isoformat(timespec='seconds'),'status':status,'stage':stage,'slug':payload.get('slug') or article.get('slug'),'title':article.get('title') or payload.get('title') or payload.get('slug') or 'Ikke navngivet kandidat','reason':reason or ('Alle gates PASS' if status=='approved' else 'Ingen begrundelse registreret'),'assessment':verdict,'assessment_text':why,'pipeline_action':adjust,'ai_usage':payload.get('ai_usage')}

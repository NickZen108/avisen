...
def validate_article(path: Path, categories: set[str], prebuild: bool) -> None:
    ...
    for i, block in enumerate(body):
        if not isinstance(block, dict) or block.get("type") not in allowed_blocks:
            err(f"{path.name}: body[{i}] har ugyldig type"); continue
        if block.get("type") == "figure":
            src = str(block.get("src", "")).strip(); alt = str(block.get("alt", "")).strip()
            if not src: err(f"{path.name}: body[{i}] figure mangler src")
            if not alt: err(f"{path.name}: body[{i}] figure mangler alt")
            if src and not (src.startswith("../img/") or re.match(r"^https?://", src)): err(f"{path.name}: body[{i}] figure src skal være ../img/... eller http(s)-URL")
            if src.startswith("../img/"):
                local=(ROOT/"docs"/"artikler"/src).resolve(); docs_root=(ROOT/"docs").resolve()
    ...
    if image is not None:
        ...
        for field in ["src", "source_url"]:
            ...
            if value and not re.match(r"^https?://", value): err(f"{path.name}: image {field} skal være http(s)-URL")
    ledger_path = ROOT / str(article.get("ledger", ""))
    if not ledger_path.exists(): err(f"{path.name}: ledger findes ikke: {article.get('ledger')}"); return
    ledger = read_json(ledger_path)
    if ledger is None: return
    sources={s.get("id"):s for s in ledger.get("sources",[]) if s.get("id")}; claims={c.get("id"):c for c in ledger.get("claims",[]) if c.get("id")}
    ...
    if article.get("status")=="published":
        ...
        if not published: err(f"{path.name}: published_at mangler")
            ...
            if dt and dt.astimezone(timezone.utc)>datetime.now(timezone.utc)+timedelta(minutes=5): err(f"{path.name}: published_at ligger i fremtiden")
        if article.get("updated_at"): parse_iso(article["updated_at"],f"{path.name}.updated_at")
        if not prebuild:
            output=ROOT/"docs"/"artikler"/f"{slug}.html"
            if not output.exists(): err(f"{path.name}: genereret HTML mangler")
            else:
                text=output.read_text(encoding="utf-8")
                if not text.startswith("<!-- GENERATED FROM content/articles/"): err(f"{path.name}: HTML mangler generated-marker")
                if len(re.findall(r"<h1(?:\s|>)",text,flags=re.I))!=1: err(f"{path.name}: genereret HTML skal have præcis én H1")

def validate_no_new_handwritten_html(prebuild: bool) -> None:
    legacy=set(); legacy_file=ROOT/"config"/"legacy-articles.txt"
    if legacy_file.exists(): legacy={x.strip() for x in legacy_file.read_text(encoding="utf-8").splitlines() if x.strip() and not x.startswith("#")}
    structured=set()
    for path in (ROOT/"content"/"articles").glob("*.json"):
        if path.name.startswith("_"): continue
        data=read_json(path)
        if data and data.get("slug"): structured.add(f"{data['slug']}.html")
    for html_path in (ROOT/"docs"/"artikler").glob("*.html"):
        if html_path.name not in legacy and html_path.name not in structured: err(f"ny håndskrevet HTML er forbudt: docs/artikler/{html_path.name}")
        if html_path.name in structured and not prebuild:
            text=html_path.read_text(encoding="utf-8")
            if not text.startswith("<!-- GENERATED FROM content/articles/"): err(f"struktureret artikel er ikke generator-output: {html_path.name}")
...
def main() -> int:
    ...
    if ERRORS:
        ...
        return 1
    print("QUALITY GATE: PASS"); return 0

#!/usr/bin/env python3
"""Shared rendering helpers for the single canonical build_all_v2 builder."""
...
def render_blocks(blocks: list[dict]) -> str:
    ...
    for block in blocks:
        ...
        if kind in {"p", "h2", "h3", "blockquote"}:
            ...
            out.append(f'<figure class="{" ".join(classes)}"><img src="{src}" alt="{alt}" loading="lazy" decoding="async">{figcaption}</figure>')
        else:
...
def build_article(path: Path) -> None:
    ...
    if article.get("followup_type") == "video" and video.get("provider") == "youtube" and video.get("id"):
        ...
        credit_html = f'<a href="{esc(source_url)}" rel="nofollow noopener">{credit}</a>' if source_url else credit
        ...
        article_image_html = '<figure class="lead-fig">' + f'<img src="{esc(image["src"])}" alt="{esc(image.get("alt", ""))}" style="height:auto;max-height:none;object-fit:contain"><figcaption>{caption}</figcaption></figure>'

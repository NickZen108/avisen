...
def dk_time(value):
    ...
    if not parsed:
        return 'ikke endnu'
    return parsed.astimezone(DK).strftime('%d-%m-%Y kl. %H:%M:%S')
...
def report_block(title, rows, open_default=False):
    ...
    table = f'<table><thead><tr><th>Tid (dansk)</th><th>Artikel</th><th>Resultat</th><th>Hvorfor</th><th>Vurdering</th></tr></thead><tbody>{body}</tbody></table>'
    if open_default:
...
def main():
    ...
    generated = e(dk_time(data.get('generated_at')))

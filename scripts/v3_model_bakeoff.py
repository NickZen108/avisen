...
def model_slug(model):
    return model.replace("@cf/", "").replace("/", "__")
...
def build_writer_fixture(key):
    ...
    for s in d.get("sources", [])[:3]:
        sources.append({
            ...
            "headline_or_scope": s.get("authoritative_for") or s.get("headline"),
            "publisher": s.get("publisher_name"),
        })
    ...
    return {
        ...
        "sources": sources,
        "verified_claims": [{"id": c.get("id"), "claim": c.get("claim"), "source_ids": c.get("source_ids", [])} for c in claims],
    }
...
def build_editor_fixture(key):
    ...
    compact = {
        "fixture": key,
        "article_file": str(p),
        "title": d.get("title"), "standfirst": d.get("standfirst"),
        "body": d.get("body", []), "image": d.get("image"),
        "story_location": d.get("story_location"), "category": d.get("category"),
    }
    return compact
...
def call(model, system, user, max_tokens=700):
    ...
    body = {"messages": [{"role":"system","content":system},{"role":"user","content":user}], "max_tokens": max_tokens, "temperature": 0.2}
    started = time.time()
    try:
        r = requests.post(url, headers={"Authorization": f"Bearer {API_TOKEN}", "Content-Type":"application/json"}, json=body, timeout=180)
...
for model in WRITER_MODELS:
    for fx in writer_fixtures:
        ...
        row = {"model":model, "fixture":fx["fixture"], **res}
        results["writer"].append(row)
        out = OUT_DIR / f"writer__{model_slug(model)}__{fx['fixture']}.json"
...
for model in EDITOR_MODELS:
    for fx in editor_fixtures:
        ...
        row = {"model":model, "fixture":fx["fixture"], **res}
        results["editor"].append(row)
        out = OUT_DIR / f"editor__{model_slug(model)}__{fx['fixture']}.json"
        out.write_text(json.dumps(row, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
        print("EDITOR", model, fx["fixture"], res["ok"], res["http_status"], res["usage"])

# One shared Desk test from the same six real story dossiers.
...
for section in ("writer","editor","desk"):
    ...
    for r in results[section]:
        ...
        b["calls"] += 1; b["success"] += int(bool(r.get("ok")))
        b["input_tokens"] += r.get("usage",{}).get("input_tokens",0); b["output_tokens"] += r.get("usage",{}).get("output_tokens",0)
        b["estimated_usd"] += r.get("usage",{}).get("estimated_usd",0.0); b["elapsed_s"] += r.get("elapsed_s",0.0)

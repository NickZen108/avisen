#!/usr/bin/env python3
"""Single canonical Morgentidende builder."""
...
def resolve(i,ix):
 ...
 return {"slug":a["slug"],"category":a["category"],"title":a["title"],"standfirst":a["standfirst"],"teaser":a["standfirst"],"published_label":core.dk_label(a["published_at"]),"image_src":im.get("src",""),"image_alt":im.get("alt",""),"image_caption":im.get("caption",""),"image_credit":im.get("credit",""),"video":video,"followup_type":a.get("followup_type"),"headline_style":a.get("headline_style")}
...
def lead_followups(lead_slug,ix):
 ...
 for a in items[:4]:
  ...
  rows.append(f'<a class="lead-followup" href="{core.front_item_url(a["slug"])}"><span class="lead-followup__type">{esc(label)}</span><strong>{esc(a["title"])}</strong></a>')
...
def lead_visual(l):
 ...
 if v.get("provider")=="youtube" and v.get("id") and v.get("frontpage_hero") is True:
  return core.youtube_embed(v,autoplay=bool(v.get("frontpage_autoplay")),css_class="frontpage-video-hero")
...
def front():
 ...
 tk=resolve(s["ticker"],ix);ticker_text=str(tk.get("title") or "").strip();ticker=f'<p><a href="{core.front_item_url(tk["slug"])}">{esc(ticker_text)}</a></p>'
 l=resolve(s["lead"],ix);visual=lead_visual(l);lead_h=headline_link(l,core.front_item_url(l["slug"]),tag="h1",allow_split=True)
 ...
 for raw in s.get("stack",[]):
  x=resolve(raw,ix);pic=f'<a href="{core.front_item_url(x["slug"])}"><img src="{esc(x["image_src"])}" alt="{esc(x.get("image_alt",""))}"></a>' if x.get("image_src") else "";stack.append(f'<article class="card">{pic}<p class="section-label">{esc(x["category"])}</p>{headline_link(x,core.front_item_url(x["slug"]),tag="h2",allow_split=True)}</article>')
 stack.append("</section>");narrow=['<section class="narrow">']
 for raw in s.get("narrow",[]):
  x=resolve(raw,ix);narrow.append(f'<article><p class="section-label">{esc(x["category"])}</p>{headline_link(x,core.front_item_url(x["slug"]),tag="h2",allow_split=False)}</article>')
...
def correction_page():
 ...
 for e in sorted(data.get("entries",[]),key=lambda z:z.get("timestamp",""),reverse=True):
  slug=e["article_slug"];title=ix.get(slug,{}).get("title",slug);rows.append(f'<article><p class="section-label">{lab.get(e.get("type"),"Rettelse")} · {core.dk_label(e["timestamp"])}</p><h2><a href="artikler/{esc(slug)}.html">{esc(title)}</a></h2><p>{esc(e["summary"])}</p></article>')
...
def public_surface():
 ...
 if sm.exists():
  ...
  sm.write_text("\n".join(lines)+"\n",encoding="utf-8")
def main():
 for p in sorted(ART.glob("*.json")):core.build_article(p)
 front();correction_page();core.build_news_sitemap();core.build_sitemap();public_surface();print("Build v2 OK")
